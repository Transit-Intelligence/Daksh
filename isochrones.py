...
def accessed_stops(
    ...
    trips_df = pd.read_csv(
        os.path.join(current_dir, "GTFS_11", "trips.txt"),
    )
    stop_times_df = pd.read_csv(
        os.path.join(current_dir, "GTFS_11", "stop_times.txt"),
    )
    file_stops_df = pd.read_csv(
        os.path.join(current_dir, "GTFS_11", "stops.txt"),
        dtype={"stop_id": "str", "stop_lat": "float", "stop_lon": "float"},
    )

    file_stops_df["file"] = str(os.path.join(current_dir, "GTFS_11", "stops.txt"))
    stops_df = pd.concat([stops_df, file_stops_df], ignore_index=True)
    file_calendar_df = pd.read_csv(
        os.path.join(current_dir, "GTFS_11", "calendar.txt"),
        dtype={
            "service_id": "str",
            "monday": "str",
            "tuesday": "str",
    ...
    file_calendar_df["file"] = str(os.path.join(current_dir, "GTFS_11", "calendar.txt"))
    calendar_df = pd.concat([calendar_df, file_calendar_df], ignore_index=True)
    file_calendar_dates_df = pd.read_csv(
        os.path.join(current_dir, "GTFS_11", "calendar_dates.txt"),
